/* Avanti Vessel AI — cérebro do protótipo: atalhos, diário, respostas canônicas e voz.
   Dados do snapshot 25/09/2026 17:43 BRT (telemetria 17:43, atracado; motores desligados após 14:03, gerador após 14:07; Seakeeper ligado). Nada inventado: sem dado → SEM DADOS. */
(function () {
  ...
  var SNAP = { pos: "22°57,09'S 043°10,23'W", hora: '25/09 17:43', diesel: 'BB 28,0 % · BE 28,8 % (≈ 426 L de 1.500)', dieselHora: 'última leitura com motores ligados · 25/09 13:59' };
  ...
  var SOS_PASSOS = 'Ordem:\n1. VHF canal 16 · MAYDAY (roteiro com MMSI e a posição lida no plotter)\n2. MOB: segurar SOS/MOB na barra superior do plotter — marca a posição\n3. Incêndio na praça de máquinas: desligue motores e ventilação · Sea-Fire dispara sozinho por temperatura · alavanca manual no console, à direita do piloto · não abra antes de 15 min\n4. EPIRB no cockpit principal, à direita · automático na água ou manual';
  ...
  var ANSWERS = {
    saudacao: function (p) { return { text: 'Olá, ' + quem() + '. ' + (window.AvantiAuth && window.AvantiAuth.frase ? window.AvantiAuth.frase() : 'Onde vamos hoje?') + '\nPosso responder sobre telemetria ao vivo, manutenção, documentos, abastecimento, diário de bordo e os passos de cada equipamento — sempre citando a fonte.', src: 'Snapshot 25/09/2026 17:43 · coletor NMEA online', actions: act(p, [['Console completo', 'console'], ['FAQ de bordo', 'faq']]) }; },
    ...
    diario: function (p, ctx, q) {
      ...
      var t = 'Atracado · Rio de Janeiro · ' + SNAP.pos + ' · SOG 0,1 nós · proa 050° · motores desligados (última leitura com motores ligados 25/09 14:03: 599 rpm, neutro) · Seakeeper ligado · banco 24 V 27,88 V · vento 1,8 nós de 044° · diesel ' + SNAP.diesel + ' (' + SNAP.dieselHora + ').';
      ...
      return { text: (grava ? 'Registrado no diário de bordo · ' + n.d + ' ' + n.t + ' · ' + quem() + '\n' + (junto ? '“' + nota + '”\n' : '') + t + '\nLinha nova — nada se apaga.' : 'Não registrado — pedido vindo de link não grava no diário.\n' + (junto ? '“' + nota + '”\n' : '') + 'Resumo de agora: ' + t + '\n' + como + ', ou toque no atalho DIÁRIO DE BORDO.') + passos, src: 'Fonte: telemetria NMEA 25/09 17:43 · DIARIO_BORDO_OPERACIONAL.csv' + (pr ? ' · manual Volvo Penta 47707890' : ''), actions: act(p, acoes) };
    },
    seguro: function (p) { return { text: 'Sim, com duas ressalvas.\n• Condições: vento 1,8 nós de 044° · barômetro 1012 hPa estável · 30 satélites · banco 24 V 27,88 V em flutuação.\n• Diesel ≈ 426 L (BB 28,0 % · BE 28,8 %) → ≈ 18 h a 8,4 nós com reserva de 10 %.\n• Ressalva 1: teste das bombas de porão e alarmes vencido há 100 dias — acione cada bomba no manual antes de largar.\n• Ressalva 2: sem previsão meteorológica carregada (SEM DADOS) — confira Marinha/DHN.\nMotores desligados — última leitura com motores ligados 25/09 14:03 (marcha lenta, neutro): 599 rpm · óleo 2,7/2,8 bar · 63/61 °C · arrefecimento 76/74 °C · sem alarmes.', src: 'Fonte: telemetria 25/09 17:43 (motores 14:03) · agenda preditiva (35 tarefas) · NF-e 002925', actions: act(p, [['Ver manutenção', 'manut'], ['Checklist de saída', askHref(p, 'Checklist de saída')]]) }; },
    destinos: function (p) { return { text: '3 destinos a partir da Marina da Glória, no regime observado (8,4 nós · 20,9 L/h), só ida:\n1. Ilhas Cagarras — ≈ 7 mn · ≈ 50 min · ≈ 17 L\n2. Itaipu / Itacoatiara (Niterói) — ≈ 10 mn · ≈ 1 h 10 · ≈ 25 L\n3. Ilha Grande (Abraão) — ≈ 60 mn · ≈ 7 h · ≈ 150 L; ida e volta ≈ 300 L — cabe nos 426 L com reserva, mas sem margem para gerador e manobra: abastecer antes.\nDistâncias em linha reta pela posição atual — confirme a rota no plotter.', src: 'Fonte: posição GPS 25/09 · consumo observado 14/08–20/09 · tanques 13:59', actions: act(p, [['Autonomia', askHref(p, 'Autonomia')], ['Abastecimento', 'abast']]) }; },
    manutencao: function (p) { return { text: 'Atrasadas (3):\n• Filtros do chiller · Dometic PLC L-3527 — 186 dias (estaleiro)\n• Teste de bombas de porão e alarmes — 100 dias (estaleiro)\n• Tensões e conexões Quick VRS / SBC NRG+ — 1 dia (Lucas)\nPróximas:\n• D-19 · 14/10 — revisão programada do sistema (Lucas + Avanti)\n• D-28 · 23/10 — isolador galvânico · ProMariner FS30/FS60 (Lucas)\n• D-48 · 12/11 — inspeção de casco, anodos e zincos (Eduardo)\nRevisão dos motores: 16/01/2027 (D-113) ou 280 h — faltam 170,0 h.\nFalta registrar: agenda do Seakeeper (zinco/trocador · 3 meses/150 h).', src: 'Fonte: agenda preditiva · 35 tarefas · horímetros 25/09 14:03', actions: act(p, [['Abrir manutenção', 'manut']]) }; },
    autonomia: function (p) { return { text: '≈ 18 h · ≈ 154 mn a 8,4 nós (1.130 rpm · 20,9 L/h), com reserva de 10 %.\nA bordo ≈ 426 L de 1.500 (BB 28,0 % · BE 28,8 %) — ' + SNAP.dieselHora + '.\nEm marcha lenta (5,1 L/h) ≈ 75 h. Consumido desde os 500 L de 14/08: ≈ 186 L até 20/09 (10,2 h de motor + 22,6 h de gerador).', src: 'Fonte: telemetria (taxas dos dois motores 14/08–20/09) · NF-e 002925 — estimativa; a NF é a fonte oficial', actions: act(p, [['Abastecimento', 'abast']]) }; },
    ...
    clima: function (p) { return { text: 'Sem previsão carregada (SEM DADOS). Leitura de agora pela estação meteorológica de bordo:\n• vento verdadeiro 1,8 nós de 044°\n• barômetro 1012 hPa · estável\n• externo 22,5 °C · praça de máquinas 21,8 °C · água do mar 21,8 °C\nFonte oficial para a previsão: Marinha do Brasil — Meteoromarinha (DHN).', src: 'Fonte: telemetria NMEA 25/09 17:43', actions: act(p, [['Telemetria', 'console']]) }; },
    ...
    tanques: function (p) { return { text: 'Tanques agora:\n• água doce 65,1 %\n• águas cinzas 0,0 %\n• águas negras 11,9 %\n• diesel BB 28,0 % · BE 28,8 % (≈ 426 L) — ' + SNAP.dieselHora + '.', src: 'Fonte: telemetria NMEA 25/09 17:43 (diesel 13:59)', actions: act(p, [['Telemetria', 'console']]) }; },
    ...
    sos: function (p) { return { text: 'Emergência — abra o SOS. ' + SOS_PASSOS + '\n' + GRAVAR_AGORA, src: 'Fonte: protocolo de emergência Avanti · inventário de segurança (Sea-Fire NFD 1000M) · registro EPIRB', actions: act(p, [['Abrir SOS', 'sos'], ['Registrar no diário', 'diario']]) }; },
    oleo: function (p) { return { text: 'Se a pressão de óleo cair abaixo da faixa (manual Volvo Penta 47707890: faixa em operação acima de 1.100 rpm) ou o alarme acender, verifique nesta ordem:\n' + OLEO_PASSOS + '\nNo snapshot de ' + SNAP.hora + ' os motores estão desligados; última leitura com motores ligados 25/09 14:03 (marcha lenta, 599 rpm, neutro): óleo 2,7/2,8 bar (BB 272 · BE 276 kPa), sem alarme.\n' + GRAVAR_AGORA, src: 'Fonte: agente Anomalia · faixas do manual Volvo Penta 47707890 · telemetria 25/09 14:03', actions: act(p, [['Registrar no diário', 'diario'], ['Contatos', 'equipe']]) }; },
    oleoLeitura: function (p) { return { text: 'Motores desligados no snapshot de ' + SNAP.hora + '. Pressão do óleo na última leitura com motores ligados 25/09 14:03: 2,7/2,8 bar (BB 272 · BE 276 kPa) · 599 rpm em neutro · óleo 63/61 °C · sem alarme.\nFaixa normal: a do manual Volvo Penta 47707890, em operação acima de 1.100 rpm.\nSe cair abaixo da faixa ou o alarme acender, pergunte “pressão de óleo baixa” para o passo a passo.', src: 'Fonte: telemetria NMEA 25/09 14:03 · manual Volvo Penta 47707890', actions: act(p, [['Telemetria', 'console'], ['Pressão baixa: o que fazer', askHref(p, 'Pressão de óleo baixa: o que verificar primeiro?')]]) }; },
